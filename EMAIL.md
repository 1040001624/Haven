...
**Two read engines shipped (Proton + IMAP); the IMAP engine sends (CP-6) and has an on-device compose / reply / forward UI (CP-7); the Mail screen is multi-account (account switcher + compose "From"). The hybrid two-engine architecture is realised.** Proton (Go bridge / SOCKS) and generic IMAP (JVM / `SocketFactory`) coexist behind one `MailBackend` → one `MimeParser`, both riding the per-profile tunnel and reachable by the agent over MCP:
...
- ✅ **Multi-account — From row + account switcher** (`6d6e0cea`, device-verified). With several mail accounts connected it was unclear which account the Mail screen showed or which one a send went out from. `MailViewModel` now observes every connected `MailSessionManager` session, tracks an **active account**, and exposes a `selectAccount`; the folder-view title becomes an **account switcher** (label + engine tag — Proton vs IMAP; dropdown when >1) and compose shows a **From** row (read-only label with one account, a picker with several) — each draft carries a `fromProfileId` and `send()` routes to that account's backend. Also fixes the empty-pane-on-bare-tab-tap (defaults to a connected account). **Device-verified with two connected Dovecot accounts:** the switcher listed both (`label · IMAP`, ✓ on active) and switching reloaded that account's folders; compose's From picker listed both, and sending with **From = acct B** wrote the audit row to **acct B's** connection log (not acct A's) + the SMTP sink captured it — proving per-account routing. **Known refinement:** the active account shows on the folder list but not on the message-list/reader title (those show the folder/subject) — surfacing it there too (subtitle) is a small follow-up. Gmail/Outlook (Stage 2b) will add `emailProvider` to `SessionState` for a finer badge than the Proton/IMAP engine tag.
- ⏭ **Next:** Proton send (Go `send`: per-recipient key discovery + `CreateDraft`/`SendDraft`; stays 501 until then). Outstanding: show the active account on the message-list/reader view too; add chip recipients (plain comma-separated fields shipped); positive IMAP-over-a-*working*-tunnel is unproven (needs a remote IMAP reachable only via a live tunnel — infra we don't have on one device); the EVERY_CALL `send_mail` prompt over a *non-trusted* (WG/LAN) carrier; optional in-process GreenMail CI test for the mapping; 11-locale translation of the CP-4/CP-7 strings.
