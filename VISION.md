...
- **Sensor / IMU / camera brokers** — extend the USB + `read_logcat` host-broker pattern to the device's own senses, so an agent's model of physical state comes from the device (see "The host as a privileged peer"), converging on the unified **Devices/Bridges** manager ([docs/design/bridges.md](docs/design/bridges.md)) so each new sense lands once and lights up every sink.
