...
- The dev agent builds Haven on the workstation over SSH, pulls the fresh APK through the namespace, installs it on the very phone it is operating (`install_apk_from_backend`), then *drives the new build and diffs the screen against its expectation* to file a verdict — a release-verify cycle with no human hand on the device. The install half ships today; the drive-and-diff half now has its first tools — `capture_haven_ui` to see Haven's own rendered screen and `tap_haven_ui` / `swipe_haven_ui` to drive it — with on-device verification, text/key entry, and the addressable `resources/*` form still ahead in §1a.
...
What was missing is symmetry: the loop must also *perceive* and *drive* its own body. The first cut of both now exists as MCP tools — `capture_haven_ui` returns Haven's own rendered screen (a `PixelCopy` of the foreground window, read-only and `FLAG_SECURE`-aware), and `tap_haven_ui` / `swipe_haven_ui` inject pointer input into that window in the same pixel space the capture reports — so an agent can install a build, see the result, and drive it through the same surface a finger would. Injection is refused while a consent prompt is showing, so it can never self-confirm. What still closes the gap, all governed by the consent tiers above and recorded to the audit log: text/key entry into Haven's own fields, exposing the capture as an addressable `resources/*` resource (today it is a tool), Tier-3 standing policies so a many-tap drive-and-diff loop needn't prompt per tap, and on-device verification of the `PixelCopy`/`MotionEvent` path (it is JVM-untestable, so the tool's JSON contract is unit-tested but the pixels and taps are not yet). Until those land the loop runs under Tiers 1–2 with a human eye on the screen, and any Tier-4 act — replacing the running app, editing the constitution — is surfaced for a human's tap. The agent↔agent half of the loop is prototyped in `docs/haven_convo_prototype.py`.
...
- **Own-UI perceive + drive — first cut landed, as tools.** `capture_haven_ui` returns Haven's own rendered screen (read-only, `FLAG_SECURE`-aware) and `tap_haven_ui` / `swipe_haven_ui` inject pointer input into that window in the captured pixel space — the screen-resource + input-verb pair the self-hosting loop needed. What's still ahead in this lane: the **`resources/*` form** of that capture (file-shaped, e.g. an addressable `ui://haven/screen`, so an agent pulls a snapshot without a tool call); **text/key entry** into Haven's own fields (pointer input is the first cut); and **on-device verification** of the `PixelCopy` + `MotionEvent` paths (JVM-untestable — the tool JSON contract is unit-tested, the pixels/taps are not). Terminal and remote-desktop snapshots remain the other screen resources.
